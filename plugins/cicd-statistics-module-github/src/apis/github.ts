...
export class CicdStatisticsApiGithub implements CicdStatisticsApi {
  ...
  private async getDefaultBranch(octo: OctokitRestWithOwnerRepo) {
    const { octokit, ...ownerRepo } = octo;
    const repo = await octokit.repos.get({ ...ownerRepo });
    return repo.data.default_branch ?? 'master';
  }

  private async listWorkflowRuns(
    { octokit, ...ownerRepo }: OctokitRestWithOwnerRepo,
    pageSize = 100,
    page = 0,
    branch?: string,
  ) {
  ...
  public async fetchBuilds(options: FetchBuildsOptions): Promise<CicdState> {
    ...
    const octo = await this.githubOctokitApi.getRestForEntity(entity, ['repo']);
